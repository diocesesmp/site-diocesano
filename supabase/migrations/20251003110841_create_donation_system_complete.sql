/*
  ...
  ## 2. Security
    ...
    - Public read access for active campaigns and stripe settings
...
DROP POLICY IF EXISTS "Authenticated users can view all campaigns" ON donation_campaigns;
CREATE POLICY "Authenticated users can view all campaigns"
  ON donation_campaigns FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can insert campaigns" ON donation_campaigns;
CREATE POLICY "Authenticated users can insert campaigns"
  ON donation_campaigns FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Authenticated users can update campaigns" ON donation_campaigns;
...
DROP POLICY IF EXISTS "Authenticated users can delete campaigns" ON donation_campaigns;
...
DROP POLICY IF EXISTS "Authenticated users can view all donations" ON donations;
...
-- Policies for stripe_settings - Allow everyone to read (needed for donation flow)
DROP POLICY IF EXISTS "Anyone can view stripe settings" ON stripe_settings;
CREATE POLICY "Anyone can view stripe settings"
  ON stripe_settings FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can update stripe settings" ON stripe_settings;
...
CREATE INDEX IF NOT EXISTS idx_donations_payment_intent ON donations(stripe_payment_intent_id);
